...
download.file("http://www.openintro.net/stat/data/bdims.RData", destfile = "bdims.RData")
load("bdims.RData")
```
...
[http://www.openintro.net/stat/data/bdims.php](http://www.openintro.net/stat/data/bdims.php),
but we'll be focusing on just three columns to get started: weight in kg (`wgt`), 
height in cm (`hgt`), bicep girth in centimeters, measured when flexed as the average of right and left girths (`bic.gi`), and `sex` (`1` indicates male, `0` indicates female).
...
5.  Using the same technique, determine whether or not male bicep girths appear to 
...
6.  Write out two probability questions that you would like to answer; one 
    regarding male heights and one regarding male bicep girth.  Calculate 
    ...
    height or bicep girth, had a closer agreement between the two methods?

<div id="license">
Modified by Yann Lamontagne from a product of OpenIntro that is released under a 
