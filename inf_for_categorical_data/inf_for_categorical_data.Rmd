...
look at the estimated proportion of atheists in the Canada. In summTable 6, we see that this is 9%. We should be able to come to 
the same number using the `atheism` data.

4.  Using the command below, create a new dataframe called `can12` that contains
    only the rows in `atheism` associated with respondents to the 2012 survey 
    from the Canada. Next, calculate the proportion of atheist 
...
5.  Write out the conditions for inference to construct a 95% confidence
    interval for the proportion of atheists in the Canada in 2012.
    Are you confident all conditions are met? Justify!
...
6.  Based on the R output, what is the margin of error for the estimate of the 
    proportion of the proportion of atheists in Canada in 2012?
...
error (`me`) associated with each of these values of `p`. Lastly, we plot the two vectors 
...
11. If you refer to Table 6, you'll find that Ecuador has a sample 
...
12.  Answer the following question. As 
    always, write out the hypotheses for any tests you conduct and outline the
    status of the conditions for inference. Show all your work!

    Is there convincing evidence that Canada has seen a change in its 
    atheism index between 2005 and 2012?



<div id="license">
Modified by Yann Lamontagne from a product of OpenIntro that is released under a [Creative Commons 
